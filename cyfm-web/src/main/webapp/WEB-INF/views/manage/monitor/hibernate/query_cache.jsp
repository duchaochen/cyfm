...
<div data-table="table" class="tabbable-line">
    ...
    <div class="tab-content">
        ...
        <ul class="nav nav-pills">
            <li ${empty param.sort ? "class='active'" : ""}><a href="${ctx}/manage/monitor/hibernate/queryCache#info">默认排序</a></li>
            <li ${param.sort eq 'hitPercent' ? "class='active'" : ""}><a href="${ctx}/manage/monitor/hibernate/queryCache?sort=hitPercent#info">总命中率</a></li>
            <li ${param.sort eq 'executionCount' ? "class='active'" : ""}><a href="${ctx}/manage/monitor/hibernate/queryCache?sort=executionCount#info">执行次数</a></li>
            <li ${param.sort eq 'executionAvgRowCount' ? "class='active'" : ""}><a href="${ctx}/manage/monitor/hibernate/queryCache?sort=executionAvgRowCount#info">执行返回的平均行数</a></li>
            <li ${param.sort eq 'executionAvgTime' ? "class='active'" : ""}><a href="${ctx}/manage/monitor/hibernate/queryCache?sort=executionAvgTime#info">平均执行时间</a></li>
            <li ${param.sort eq 'executionMaxTime' ? "class='active'" : ""}><a href="${ctx}/manage/monitor/hibernate/queryCache?sort=executionMaxTime#info">最长执行时间</a></li>
        </ul>
        <table class="table table-bordered">
            <tbody>
            <%
        ...
        Arrays.sort(queries, new Comparator<String>() {
            public int compare(String q1, String q2) {
                ...
                if ("executionAvgRowCount".equals(sortBy)) {
                    long js = s1.getExecutionRowCount()==0?1:s1.getExecutionRowCount();
                    return -Long.valueOf(s1.getExecutionRowCount() / js)
                            .compareTo(Long.valueOf(s2.getExecutionRowCount() / js));
                }
