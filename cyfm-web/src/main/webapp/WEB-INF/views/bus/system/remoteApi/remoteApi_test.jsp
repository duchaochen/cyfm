...
<body>
    <div class="portlet box editBox">
        <div class="portlet-title"><span>接入API调试</span></div>
        <div class="portlet-body form">
            <div class="form-body">
                <input type="hidden" name="id" value="${entity.id}"/>
                <div class="form-group">
                    <label for="joinSystem" class="control-label">接入系统:</label>
                    <div class="controls">
                        <select id="joinSystem" name="joinSystem" class="form-control required" >
                            <option label="请选择" value=""/>
                            <c:forEach items="${joinSystems}" var="system">
                                <optgroup label="${system.sysName}">
                                    <c:forEach items="${system.remoteApiModels}" var="api">
                                        <option value="${api.id}" data-method="${api.requestMethod}" data-param='${api.paramModel}' title="${api.remark}">${api.remoteApiName}</option>
                                    </c:forEach>
                                </optgroup>
                            </c:forEach>
                        </select>
                ...
                <div class="form-group">
                    <label for="joinSystem" class="control-label">请求参数(如果没有请求参数则不填写):</label>
                    <div class="controls">
                        <textarea id="params" class="form-control" placeholder="请输入请求参数"></textarea>
                    </div>
                </div>

                <div class="form-actions">
                    <input id="submit_btn" class="btn btn-primary" type="button" value="提交请求"/>&nbsp;
                    <p class="help-block">(点击进行API请求.)</p>
                </div>
            </div>
        </div>
    </div>
    <script>
        $(function () {
            ...
            $("#submit_btn").click(function () {
                var api = $("#joinSystem").val();
                var params = $("#params").val();

                if (api == '') {
                    $cy.warn("请选择一个 API 进行调试");
                    return false;
                }
                ...
                $.ajax({
                    type: "post",
                    dataType: "text",
                    url: "${ctx}/bus/system/remoteApi/test",
                    data: {"api": api, "params": params},
        ...
        function formatResult(result){
            if (isJSON(result)){
                return "<textarea style='width:100%;height: 98%'>"+formatJson(result)+"</textarea>";
            }else{
                return "<textarea style='width:100%;height: 98%'>"+HTMLFormat(result.substring(result.lastIndexOf("<body"),result.lastIndexOf("</html>")))+"</textarea>";
            }
        }
        function isJSON(str) {
