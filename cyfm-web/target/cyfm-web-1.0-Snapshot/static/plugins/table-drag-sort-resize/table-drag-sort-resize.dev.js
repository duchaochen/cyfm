...
(function () {
    ...
    function sort(cell, table) {
        //TODO 如果点击的不是TH则不进行排序,有优化余地,可以自动找父级TH
        if (cell.tagName !== 'TH') {
            return;
        }
    ...
    (function () {
        ...
        DragSortHandler.prototype._mousePrepareDrag = function (event) {
            ...
            //TODO 加上边线避免表格无边框效果 top+1 让出了上边线位
            back.style.top = backTop + 1 + 'px';
            ...
            for (var i = 0; i < this.nc; i++) {
                ...
                if (globalOptions.fixed && this.nc == i + 1) {
                    elementBase.style.width = elementBaseWidth - 1 + 'px';
                }
        ...
        DragSortHandler.prototype._mouseExecuteClick = function (event) {
            ...
            if (globalOptions.sort.localEnable) {
                ...
                if (this.options.restoreState)
                    saveState('table-drag-sort-resize', this.table, 'sort', {
                        index: index,
