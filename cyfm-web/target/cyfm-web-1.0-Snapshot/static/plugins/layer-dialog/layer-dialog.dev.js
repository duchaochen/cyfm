...
//创建骨架
    Class.pt.creat = function () {
        ...
        that.vessel(conType, function (html, titleHTML) {
            ...
            that.layero.data("layerObj", that);
...
//计算坐标
    Class.pt.offset = function () {
        ...
        layero.css({top: that.offsetTop + config.topRevised, left: that.offsetLeft + config.leftRevised});
    };
...
//Tips
    Class.pt.tips = function () {
        ...
        var layArea = [layero.outerWidth(), layero.outerHeight()], follow = $(config.follow),
            topRevised = config.topRevised, leftRevised = config.leftRevised;
        if (!follow[0]) follow = $('body');

        var goal = {
            width: follow.outerWidth(),
            height: follow.outerHeight(),
            top: follow.offset().top + topRevised,
            left: follow.offset().left + leftRevised,
    ...
    Class.pt.move = function () {
        ...
        $(config.move).on('mousedown', function (M) {
            ...
            if ($(this).attr('move') === 'ok') {
                ...
                var xx = conf.layero.offset().left, yy = conf.layero.offset().top, ww = conf.layero.outerWidth() - 6,
...
//通过index得到
    layer.getFrameLayerObj = function (index) {
        return $('#' + doms[0] + index).data("layerObj");
    };
...
//关闭layer总方法
    layer.close = function (index) {
        ...
        if (!layero[0]) return;
        //如果被手动关闭，则不再进行关闭回调。
        layer.getFrameLayerObj(index).config.timeClose = undefined;
