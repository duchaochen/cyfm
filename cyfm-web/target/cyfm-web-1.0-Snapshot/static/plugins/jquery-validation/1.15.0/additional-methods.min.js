...
}());a.validator.addMethod("accept",function(g,d,j){var h=typeof j==="string"?j.replace(/\s/g,""):"image/*",f=this.optional(d),c,b,e;
if(f){return f
}if(a(d).attr("type")==="file"){h=h.replace(/[\-\[\]\/\{\}\(\)\+\?\.\\\^\$\|]/g,"\\$&").replace(/,/g,"|").replace("/*","/.*");
if(d.files&&d.files.length){e=new RegExp(".?("+h+")$","i");
for(c=0;c<d.files.length;
c++){b=d.files[c];
...
}var f=g.replace(/ /g,""),e=0,b=f.length,i,c,h;
for(i=0;i<b;
i++){c=b-i;
h=f.substring(i,i+1);
e=e+c*h}return e%11===0
...
j+=b;return(d[8].toString()===String.fromCharCode(64+b)||d[8].toString()===j.charAt(j.length-1))
}return false
},"Please specify a valid CIF number.");
a.validator.addMethod("cpfBR",function(g){g=g.replace(/([~!@#$%^&*()_+=`{}\[\]\-|\\:;'<>,.\/? ])+/g,"");
if(g.length!==11){return false
}var f=0,e,d,c,b;
e=parseInt(g.substring(9,10),10);
d=parseInt(g.substring(10,11),10);
c=function(i,j){var h=(i*10)%11;
if((h===10)||(h===11)){h=0
}return(h===j)
};if(g===""||g==="00000000000"||g==="11111111111"||g==="22222222222"||g==="33333333333"||g==="44444444444"||g==="55555555555"||g==="66666666666"||g==="77777777777"||g==="88888888888"||g==="99999999999"){return false
}for(b=1;b<=9;
b++){f=f+parseInt(g.substring(b-1,b),10)*(11-b)
}if(c(f,e)){f=0;
for(b=1;b<=10;
b++){f=f+parseInt(g.substring(b-1,b),10)*(12-b)
}return c(f,d)
}return false
...
a.validator.addMethod("dateITA",function(f,d){var b=false,h=/^\d{1,2}\/\d{1,2}\/\d{4}$/,j,e,c,g,i;
if(h.test(f)){j=f.split("/");
e=parseInt(j[0],10);
c=parseInt(j[1],10);
g=parseInt(j[2],10);
i=new Date(Date.UTC(g,c-1,e,12,0,0,0));
if((i.getUTCFullYear()===g)&&(i.getUTCMonth()===c-1)&&(i.getUTCDate()===e)){b=true
}else{b=false
}}else{b=false
...
n=b[d];if(typeof n!=="undefined"){k=new RegExp("^[A-Z]{2}\\d{2}"+n+"$","");
...
jQuery.validator.addMethod("notEqualTo",function(c,b,d){return this.optional(b)||!a.validator.methods.equalTo.call(this,c,b,d)
...
}return g},a.validator.format("Please fill at least {0} of these fields."));
a.validator.addMethod("skip_or_fill_minimum",function(f,e,c){var b=a(c[1],e.form),i=b.eq(0),d=i.data("valid_skip")?i.data("valid_skip"):a.extend({},this),h=b.filter(function(){return d.elementValue(this)
}).length,g=h===0||h>=c[0];
i.data("valid_skip",d);
if(!a(e).data("being_validated")){b.data("being_validated",true);
b.each(function(){d.element(this)
});b.data("being_validated",false)
}return g},a.validator.format("Please either skip these fields or fill at least {0} of them."));
a.validator.addMethod("stateUS",function(h,e,d){var g=typeof d==="undefined",b=(g||typeof d.caseSensitive==="undefined")?false:d.caseSensitive,i=(g||typeof d.includeTerritories==="undefined")?false:d.includeTerritories,c=(g||typeof d.includeMilitary==="undefined")?false:d.includeMilitary,f;
if(!i&&!c){f="^(A[KLRZ]|C[AOT]|D[CE]|FL|GA|HI|I[ADLN]|K[SY]|LA|M[ADEINOST]|N[CDEHJMVY]|O[HKR]|PA|RI|S[CD]|T[NX]|UT|V[AT]|W[AIVY])$"
}else{if(i&&c){f="^(A[AEKLPRSZ]|C[AOT]|D[CE]|FL|G[AU]|HI|I[ADLN]|K[SY]|LA|M[ADEINOPST]|N[CDEHJMVY]|O[HKR]|P[AR]|RI|S[CD]|T[NX]|UT|V[AIT]|W[AIVY])$"
}else{if(i){f="^(A[KLRSZ]|C[AOT]|D[CE]|FL|G[AU]|HI|I[ADLN]|K[SY]|LA|M[ADEINOPST]|N[CDEHJMVY]|O[HKR]|P[AR]|RI|S[CD]|T[NX]|UT|V[AIT]|W[AIVY])$"
